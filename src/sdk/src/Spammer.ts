...
export class Spammer {
  ...
  protected async spam(loop: boolean) {
    ...
    try {
      ...
      if (this.txsSinceRotate >= TXS_UNTIL_ROTATE) {
        ...
        this.event({
          type: "debug",
          msg: `Rotating to next RPC: ${nextClient.rpcUrl}`,
        });
      ...
      hasToRegister =
        counters.register !== null && !counters.register.registered;
      ...
      // User ran out of gas
        ...
        errStr.includes("No valid gas coins found for the transaction") ||
      ...
      // The validator didn't pick up the object changes yet. Happens sometimes when changing RPCs.
        ...
        errStr.includes("not available for consumption")
      ) {
        const retryMsg = `Retrying in ${SLEEP_MS_AFTER_OBJECT_NOT_READY / 1000} seconds`;
        this.event({
          type: "debug",
          msg: `Validator didn't sync yet. ${retryMsg}. RPC: ${this.getSpamClient().rpcUrl}.`,
        });
      ...
      // Network error
        ...
        this.event({
          type: "warn",
          msg: `Network error. ${retryMsg}. Details: ${errStr}`,
        });
      ...
      // Consensus error
        ...
        this.event({
          type: "warn",
          msg: `Consensus error. Details: ${errStr}`,
        });
        this.status = "stopping";
      }
      // Unexpected error
        ...
        this.event({
          type: "warn",
          msg: `Unexpected error. ${retryMsg}. Details: ${errStr}`,
        });
        this.txsSinceRotate += 17; // spend less time on failing RPCs
  ...
  protected async refetchData(): Promise<void> {
    ...
    if (this.lastTxDigest) {
      this.event({
        type: "debug",
        msg: `Waiting for tx: ${this.lastTxDigest}`,
      });
      await this.getIotaClient().waitForTransaction({
  ...
  protected async incrementUserCounter(
    ...
    this.userCounters.current.ref = resp.effects.mutated!.find(
      (mutatedObj) => mutatedObj.reference.objectId == counterRef.objectId,
    )!.reference;
  }
